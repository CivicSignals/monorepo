# Third-Party Notices

CivicSignals is licensed under **AGPL-3.0-only**. This file lists the
third-party packages distributed with the backend API (`apps/api`) and their
licenses, for attribution.

Regenerate with `./scripts/gen-notice.sh`; CI (`license-check`, TODO A3)
fails if this file is stale or a disallowed license appears.

## Python (runtime dependencies)

| Name                      | License                                            | URL                                                            |
|---------------------------|----------------------------------------------------|----------------------------------------------------------------|
| Mako                      | MIT License                                        | https://www.makotemplates.org/                                 |
| MarkupSafe                | BSD-3-Clause                                       | https://github.com/pallets/markupsafe/                         |
| PyJWT                     | MIT                                                | https://github.com/jpadilla/pyjwt                              |
| PyYAML                    | MIT License                                        | https://pyyaml.org/                                            |
| SQLAlchemy                | MIT                                                | https://www.sqlalchemy.org                                     |
| alembic                   | MIT                                                | https://alembic.sqlalchemy.org                                 |
| amqp                      | BSD License                                        | http://github.com/celery/py-amqp                               |
| annotated-doc             | MIT                                                | https://github.com/fastapi/annotated-doc                       |
| annotated-types           | MIT License                                        | https://github.com/annotated-types/annotated-types             |
| anyio                     | MIT                                                | https://anyio.readthedocs.io/en/stable/versionhistory.html     |
| asyncpg                   | Apache-2.0                                         | UNKNOWN                                                        |
| attrs                     | MIT                                                | https://www.attrs.org/en/stable/changelog.html                 |
| bcrypt                    | Apache Software License                            | https://github.com/pyca/bcrypt/                                |
| beautifulsoup4            | MIT License                                        | https://www.crummy.com/software/BeautifulSoup/bs4/             |
| billiard                  | BSD License                                        | https://github.com/celery/billiard                             |
| boto3                     | Apache-2.0                                         | https://github.com/boto/boto3                                  |
| botocore                  | Apache-2.0                                         | https://github.com/boto/botocore                               |
| celery                    | BSD-3-Clause                                       | https://docs.celeryq.dev/                                      |
| certifi                   | Mozilla Public License 2.0 (MPL 2.0)               | https://github.com/certifi/python-certifi                      |
| click                     | BSD-3-Clause                                       | https://github.com/pallets/click/                              |
| click-didyoumean          | MIT License                                        | https://github.com/click-contrib/click-didyoumean              |
| click-plugins             | BSD License                                        | https://github.com/click-contrib/click-plugins                 |
| click-repl                | MIT                                                | https://github.com/untitaker/click-repl                        |
| dnspython                 | ISC License (ISCL)                                 | https://www.dnspython.org                                      |
| email-validator           | The Unlicense (Unlicense)                          | https://github.com/JoshData/python-email-validator             |
| fastapi                   | MIT                                                | https://github.com/fastapi/fastapi                             |
| greenlet                  | MIT AND PSF-2.0                                    | https://greenlet.readthedocs.io                                |
| h11                       | MIT License                                        | https://github.com/python-hyper/h11                            |
| httpcore                  | BSD-3-Clause                                       | https://www.encode.io/httpcore/                                |
| httptools                 | MIT                                                | https://github.com/MagicStack/httptools                        |
| httpx                     | BSD License                                        | https://github.com/encode/httpx                                |
| idna                      | BSD-3-Clause                                       | https://github.com/kjd/idna                                    |
| jmespath                  | MIT License                                        | https://github.com/jmespath/jmespath.py                        |
| jsonschema                | MIT                                                | https://github.com/python-jsonschema/jsonschema                |
| jsonschema-specifications | MIT                                                | https://github.com/python-jsonschema/jsonschema-specifications |
| kombu                     | BSD-3-Clause                                       | https://kombu.readthedocs.io                                   |
| numpy                     | BSD-3-Clause AND 0BSD AND MIT AND Zlib AND CC0-1.0 | https://numpy.org                                              |
| packaging                 | Apache-2.0 OR BSD-2-Clause                         | https://github.com/pypa/packaging                              |
| passlib                   | BSD                                                | https://passlib.readthedocs.io                                 |
| pgvector                  | MIT                                                | https://github.com/pgvector/pgvector-python                    |
| prompt_toolkit            | BSD License                                        | https://github.com/prompt-toolkit/python-prompt-toolkit        |
| pydantic                  | MIT                                                | https://github.com/pydantic/pydantic                           |
| pydantic-settings         | MIT                                                | https://github.com/pydantic/pydantic-settings                  |
| pydantic_core             | MIT                                                | https://github.com/pydantic                                    |
| python-dateutil           | Apache Software License; BSD License               | https://github.com/dateutil/dateutil                           |
| python-dotenv             | BSD-3-Clause                                       | https://github.com/theskumar/python-dotenv                     |
| python-multipart          | Apache-2.0                                         | https://github.com/Kludex/python-multipart                     |
| redis                     | MIT                                                | https://github.com/redis/redis-py                              |
| referencing               | MIT                                                | https://github.com/python-jsonschema/referencing               |
| rpds-py                   | MIT                                                | https://github.com/crate-py/rpds                               |
| s3transfer                | Apache Software License                            | https://github.com/boto/s3transfer                             |
| six                       | MIT License                                        | https://github.com/benjaminp/six                               |
| soupsieve                 | MIT                                                | https://github.com/facelessuser/soupsieve                      |
| starlette                 | BSD-3-Clause                                       | https://github.com/Kludex/starlette                            |
| structlog                 | MIT OR Apache-2.0                                  | https://github.com/hynek/structlog/blob/main/CHANGELOG.md      |
| tenacity                  | Apache Software License                            | https://github.com/jd/tenacity                                 |
| typing-inspection         | MIT                                                | https://github.com/pydantic/typing-inspection                  |
| typing_extensions         | PSF-2.0                                            | https://github.com/python/typing_extensions                    |
| tzdata                    | Apache-2.0                                         | https://github.com/python/tzdata                               |
| tzlocal                   | MIT License                                        | https://github.com/regebro/tzlocal/blob/master/CHANGES.txt     |
| urllib3                   | MIT                                                | https://github.com/urllib3/urllib3/blob/main/CHANGES.rst       |
| uvicorn                   | BSD-3-Clause                                       | https://uvicorn.dev/                                           |
| uvloop                    | Apache Software License; MIT License               | UNKNOWN                                                        |
| vine                      | BSD License                                        | https://github.com/celery/vine                                 |
| watchfiles                | MIT License                                        | https://github.com/samuelcolvin/watchfiles                     |
| websockets                | BSD-3-Clause                                       | https://github.com/python-websockets/websockets                |
