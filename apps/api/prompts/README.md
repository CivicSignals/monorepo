...
The gateway adopts a prompt's declared `task` only when the caller left `task`
at its default; an explicit `task=` always wins. `task` then routes through
`TaskModelPolicy` (so a self-hoster's settings overrides still govern the actual
provider/model), and an explicit `provider`/`model` (e.g. doc 19 §5.1
Haiku→Sonnet escalation) overrides the model independently. `model_hint` is
purely advisory — it is exposed on the resolved `Prompt` for documentation and
tooling but never bypasses the policy. Per doc 19, classify/query-rewrite
prompts hint Haiku; extraction prompts hint Sonnet.
