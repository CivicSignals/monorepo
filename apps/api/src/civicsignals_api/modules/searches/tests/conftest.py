"""Test fixtures for the searches module (H1 saved searches).

Saved searches use Postgres-specific columns (JSONB), so this builds an engine
from ``DATABASE_DIRECT_URL``/``DATABASE_URL`` and skips when neither is set — the
same pattern as the icp/accounts conftest. CI provides a pgvector Postgres
service, so the flow runs there.
...
from civicsignals_api.modules.searches.models import (  # noqa: F401  register tables
    SavedSearch,
)
